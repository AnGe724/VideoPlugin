...
{
    _progress = (float)fmax(0.0f, fmin(1.0f, newProgress));
...
+ (void)initialize
{
    if (self == [TYMProgressBarView class]) {
        ...
        [appearance setBarBackgroundColor:[UIColor lightGrayColor]];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.barInnerBorderColor && self.barInnerBorderWidth > 0.0f) {
        ...
        currentRect.size.width = (float)fmax(currentRect.size.width, 2 * radius);
    ...
    if (self.barFillColor) {
        ...
        if (!hasInnerBorder) {
            currentRect.size.width *= self.progress;
            currentRect.size.width = (float)fmax(currentRect.size.width, 15);
